...
And('I enter the atendimento date {string}') do |string|
  ...
  fill_in 'atendimento_data_hora', with: string
end

And('I select atendimento time with {string}:{string}') do |time, minutes|
  select time, from: 'atendimento_data_hora_4i'
  select minutes, from: 'atendimento_data_hora_5i'
end
